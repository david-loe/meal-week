...
# create backup
docker exec meal-week-db-1 sh -c 'exec mongodump -d week_of_meals --archive' > $SCRIPTPATH/db-$(date +"%Y%m%dT%H%M").archive

# delete all backups older than 50 days
find . -type f -mtime +50 -name '*.archive' -execdir rm -- '{}' \;
